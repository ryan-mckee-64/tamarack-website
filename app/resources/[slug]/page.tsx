import Link from "next/link";
import { notFound } from "next/navigation";
import type { Metadata } from "next";
import { plannedItems, getPlannedItem, groupForItem } from "@/lib/resources";

export function generateStaticParams() {
  return plannedItems().map((i) => ({ slug: i.slug }));
}

export async function generateMetadata({
  params,
}: {
  params: Promise<{ slug: string }>;
}): Promise<Metadata> {
  const { slug } = await params;
  const item = getPlannedItem(slug);
  if (!item) return { title: "Resources | Tamarack Industries" };
  return { title: `${item.label} | Tamarack Industries` };
}

export default async function PlannedResourcePage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const { slug } = await params;
  const item = getPlannedItem(slug);
  if (!item) notFound();

  const group = groupForItem(slug);

  return (
    <main>
      <section className="mx-auto max-w-3xl px-6 py-20 md:px-10">
        <Link
          href="/resources"
          className="text-sm font-semibold text-[color:var(--ink-dim)] transition hover:text-[color:var(--orange)]"
        >
          Back to resources
        </Link>

        {group && (
          <p className="tech-label mt-8 text-[color:var(--ember)]">
            {group.label}
          </p>
        )}
        <h1 className="font-display mt-3 text-4xl font-extrabold tracking-[-0.03em] text-[color:var(--ink)] sm:text-5xl">
          {item.label}
        </h1>
        <p className="mt-5 text-lg leading-relaxed text-[color:var(--ink-dim)]">
          {item.description}
        </p>

        <div className="mt-10 rounded-2xl border border-[color:var(--line)] bg-[var(--surface)] p-8">
          <p className="font-semibold text-[color:var(--ink)]">
            This resource is being prepared
          </p>
          <p className="mt-3 text-sm leading-relaxed text-[color:var(--ink-dim)]">
            We are putting this section together now. In the meantime our
            service team can answer questions directly, and the manual library
            covers operating and parts information for every product line.
          </p>
          <div className="mt-6 flex flex-wrap gap-3">
            <Link
              href="/manuals"
              className="brand-gradient rounded-full px-6 py-2.5 text-sm font-semibold text-white transition-opacity hover:opacity-90"
            >
              Browse manuals
            </Link>
            <Link
              href="/contact"
              className="rounded-full border border-[color:var(--line-strong)] px-6 py-2.5 text-sm font-semibold text-[color:var(--ink)] transition hover:border-[color:var(--orange)] hover:text-[color:var(--orange)]"
            >
              Contact our team
            </Link>
          </div>
        </div>
      </section>
    </main>
  );
}